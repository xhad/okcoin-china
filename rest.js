...
var OKCoin = function OKCoin(api_key,
	secret, server, timeout) {
...
var headers = {
	"User-Agent": "OKCoin Javascript API Client"
};

OKCoin.prototype.privateRequest =
	function(method, params, callback) {
		var functionName =
			'OKCoin.privateRequest()',
			self = this;

		if (!this.api_key || !this.secret) {
			var error = new VError(
				'%s must provide api_key and secret to make this API request.',
				functionName);
		...
		if (!callback || typeof(callback) !=
			'function') {
			var error = new VError(
				'%s third parameter needs to be a callback function',
				functionName);
			return callback(error);
		}
		...
		var requestDesc = util.format(
			'%s request to url %s with method %s and params %s',
 ...
 */
OKCoin.prototype.signMessage = function getMessageSignature(
	params) {
	var formattedParams =
		formatParameters(params);

	// append secret key value pair
	formattedParams += '&secret_key=' +
		this.secret;
...
OKCoin.prototype.publicRequest =
	function(method, params, callback) {
		var functionName =
			'OKCoin.publicRequest()';
...
function executeRequest(options,
	requestDesc, callback) {
	var functionName =
		'OKCoin.executeRequest()';
	...
	request(options, function(err,
		...
		if (options.json && !_.isObject(
				data)) {
			error = new VError(
				'%s could not parse response from %s\nResponse: %s',
				functionName, requestDesc, data);
		}

		if (_.has(returnObject,
				'error_code')) {
			var errorMessage = mapErrorMessage(
				returnObject.error_code);

			error = new VError(
				'%s %s returned error code %s, message: "%s"',
				functionName,
				requestDesc, returnObject.error_code,
				errorMessage);
...
OKCoin.prototype.getTicker = function getTicker(
	callback, symbol) {
...
OKCoin.prototype.getDepth = function getDepth(
	callback, symbol, size, merge) {
	var params = {
		symbol: symbol,
		size: 200,
		merge: 1
	...
	if (!_.isUndefined(size)) params.size =
		size;
	if (!_.isUndefined(merge)) params.merge =
		merge;

	this.publicRequest('depth', params,
		callback);
};

OKCoin.prototype.getTrades = function getTrades(
	callback, symbol, since) {
	var params = {
		symbol: symbol
	};
	if (since) params.since = since;

	this.publicRequest('trades', params,
		callback);
};

OKCoin.prototype.getKline = function getKline(
	callback, symbol, type, size, since) {
	var params = {
		symbol: symbol
	};
	...
	this.publicRequest('kline', params,
		callback);
};

OKCoin.prototype.getLendDepth =
	function getLendDepth(callback, symbol) {
		this.publicRequest('kline', {
			symbol: symbol
		}, callback);
	};
...
OKCoin.prototype.getUserInfo = function getUserInfo(
	callback) {
	this.privateRequest('userinfo', {},
		callback);
};

OKCoin.prototype.addTrade = function addTrade(
	callback, symbol, type, amount, price
) {
	var params = {
		symbol: symbol,
		type: type
	...
	this.privateRequest('trade', params,
		callback);
};

OKCoin.prototype.addBatchTrades =
	function addBatchTrades(callback,
		symbol, type, orders) {
...
OKCoin.prototype.cancelOrder = function cancelOrder(
	callback, symbol, order_id) {
...
OKCoin.prototype.getOrderInfo =
	function getOrderInfo(callback, symbol,
		order_id) {
...
OKCoin.prototype.getOrdersInfo =
	function getOrdersInfo(callback,
		symbol, type, order_id) {
...
OKCoin.prototype.getAccountRecords =
	function getAccountRecords(callback,
		symbol, type, current_page,
		page_length) {
...
OKCoin.prototype.getTradeHistory =
	function getTradeHistory(callback,
		symbol, since) {
...
OKCoin.prototype.getOrderHistory =
	function getOrderHistory(callback,
		symbol, status, current_page,
		page_length) {
...
OKCoin.prototype.addWithdraw = function addWithdraw(
	callback, symbol, chargefee,
	trade_pwd, withdraw_address,
	withdraw_amount) {
...
OKCoin.prototype.cancelWithdraw =
	function cancelWithdraw(callback,
		symbol, withdraw_id) {
...
function mapErrorMessage(error_code) {
	...
	if (!errorCodes[error_code]) {
		return 'Unknown OKCoin error code: ' +
			error_code;
	}
